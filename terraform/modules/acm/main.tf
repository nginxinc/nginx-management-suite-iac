resource "null_resource" "make_htpasswd_file" {
  ...
  provisioner "local-exec" {
    command = "htpasswd -c -b ${path.cwd}/.acm_htpasswd ${var.admin_user} '${var.admin_passwd}'"
    when    = create
  }
}
...
data "local_file" "htpasswd_file" {
  ...
  filename = "${path.cwd}/.acm_htpasswd"
}
...
data "template_cloudinit_config" "acm_cloud_init" {
  depends_on = [
    data.local_file.ssh_pub_file,
    data.local_file.htpasswd_file
  ]
  ...
  part {
    ...
    content = templatefile("${path.module}/acm_cloud_init.tmpl", {
